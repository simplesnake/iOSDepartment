...
public class ConfirmViewController: UIViewController {
    ...
    private func prepareScrollView() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + .milliseconds(10), execute: {
            ...
            let scale = self.calculateMinimumScale(self.view.frame.size)
...
extension ConfirmViewController: CropOverlayDelegate {
    ...
    func didMoveCropOverlay(newFrame: CGRect) {
        ...
        cropOverlayHeightConstraint.constant = newFrame.size.width
    }
}
...
extension PHAsset {
    ...
    var image: UIImage? {
        let size: CGSize = CGSize(width: pixelWidth, height: pixelHeight)
        ...
        manager.requestImage(for: self, targetSize: size, contentMode: .aspectFit, options: options, resultHandler: {(result, info)->Void in
            thumbnail = result!
        })
        return thumbnail
    }
